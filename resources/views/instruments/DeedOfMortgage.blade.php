...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item" aria-current="page"> {{ __('Instrument Registration Module') }} {{$title}}</li>
@endsection
@push('script-page')
...
@section('content')
    ...
    <div class="container mx-auto mt-4 p-4">
        ...
        <form class="space-y-6" action="{{ route('instruments.store') }}" method="POST">
            @csrf
            <!-- File Information -->
            ...
            <div class="bg-gray-50 p-4 rounded-md">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="instructionName" name="instructionName" class="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value="{{$title}}" readonly>
                    </div>
                    <div>
                        <label for="instrumentDate" class="block text-sm font-medium text-gray-700 mb-1">Instrument Date</label>
                        <input type="date" id="instrumentDate" name="instrumentDate" class="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value="{{ date('Y-m-d') }}">
                    </div>
                </div>
            </div>
   ...
   <br>
    ...
    <script>
        ...
        {
            const rootTitleRegNo = '{{$RootTitleRegNo}}';
